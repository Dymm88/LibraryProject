...
async def update_author(author_id: int, replace_author: AuthorCreate,
    ...
    mod_author = await AuthorCRUD(session).update(author_id=author_id, author_data=replace_author)
    return mod_author
...
async def partial_update_author(author_id: int, replace_author: AuthorCreate,
                                session: AsyncSession = Depends(db_handler.get_db)):
    mod_author = await AuthorCRUD(session).partial_update(author_id=author_id, author_data=replace_author)
    return mod_author
